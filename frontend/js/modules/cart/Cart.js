import { basketIcon, basket } from "./template.js";
import { cart } from "../../repository/storage.js";
...
export class Cart {
  ...
  onProductQuantityChange(isIncrease, e) {
    ...
    this.item = this.items.find((elem) => elem._id === e.target.value);
  ...
  addItem(item) {
    const product = this.items.find((elem) => elem._id === item._id);
  ...
  onDelete(e) {
    ...
    this.items = this.items.filter((elem) => elem._id !== e.target.value);
